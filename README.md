...
| Item2Vec         | 0.0441 | 0.0908 | 0.1863 | 0.0326 |
| ImplicitBPR      | 0.0167 | 0.0440 | 0.0692 | 0.0110 |
...
| Item2Vec         | −37.4% | −38.4% | −35.6% |
| ImplicitBPR      | −48.9% | −29.3% | −35.3% |
...
`Item2Vec` занял среднюю позицию (Hit@5 = 7.1% на Test). Модель улавливает контекст совместных покупок, но уступает прямому Lift'у - корзины слишком короткие (2-4 товара) для качественного обучения Word2Vec эмбеддингов. При этом на Hit@20 Item2Vec приближается к CooccurrenceLift (28.5% vs 29.6%), что говорит о хорошем recall на дальних позициях. Заметим, что результаты Item2Vec немного варьируются между запусками из-за многопоточности Word2Vec (workers>1)
